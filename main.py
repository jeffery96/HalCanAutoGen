...
with open('./hacg_can.h', 'w') as hacg_can_h_file:
    ...
    for msg in can0_dbc.messages:
        if 'VCU' in msg.senders:
            hacg_can_h_file.write(f'void Can0Msg_Pack_{hex(msg.frame_id)}(void)\n'
                                  f'{{\n')
            # Clear Message Buffer first!
            hacg_can_h_file.write(f'    /*Clear Message Buffer first!*/\n')
            for i in range(8):
                hacg_can_h_file.write(f'    {msg.name}_buf[{i}] = 0x00u;\n')
            ...
            for sig in msg.signals:
                ...
                hacg_can_h_file.write(f'    /*signalName:pfc_{sig.name}, DataOrder:{sig.byte_order}, startBit: {sig.start}, length: {sig.length} */\n')
                ...
                for i, j in enumerate(range(start, end, 8)):
                    ...
                    hacg_can_h_file.write(f'    {msg.name}_buf[{byte_index}] |= ((uint8_t)(pfc_{sig.name} {shift_dir} {shift_num}u) & {filter_bit_in_hex}u);\n')

            hacg_can_h_file.write(f'}}\n\n')
        else:
            ...
            hacg_can_h_file.write(f'void Can0Msg_Unpack_{hex(msg.frame_id)}(void)\n'
                                  f'{{\n')
            for sig in msg.signals:
                sig_type = 'uint8_t' if sig.length <= 8 else 'uint16_t' if sig.length <= 16 else 'uint32_t'
                start = sig.start
                ...
                for i, j in enumerate(range(start, end, 8)):
                    ...
                    shift_num = shift_num if i == 0 else 8
                    # print(shift_num)
                    hacg_can_h_file.write(f'    pfc_{sig.name} = (({sig_type})({msg.name}_buf[{byte_index}] & {filter_bit_in_hex}u) {shift_dir} {shift_num}u);\n')
                    # hacg_can_h_file.write(
                    #     f'    {msg.name}_buf[{byte_index}] |= ((uint8_t)(pfc_{sig.name} {shift_dir} {i * 8}u) & {filter_bit}u);\n')
            hacg_can_h_file.write(f'}}\n\n')
    hacg_can_h_file.write('\n')
