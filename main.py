...
with open('./io_vars.h', 'w') as iovars_h_file:
    ...
    for msg in messages:
        for sig in msg.signals:
            sig_type = 'extern uint8_t' if sig.length <= 8 else 'extern uint16_t' if sig.length <= 16 else 'extern ' \
                                                                                                           'uint32_t '
            sig_initvalue = hex(int(-sig.offset / sig.scale)) + \
                            'u' if sig.initial is None else hex(sig.initial) + 'u'
            iovars_h_file.write(
    ...
    for msg in messages:
        if 'VCU' in msg.senders:
            hacg_can_c_file.write(
                f'void Can0Msg_Pack_{hex(msg.frame_id)}(void)\n'
                f'{{\n')
            ...
            for sig in msg.signals:
                ...
                filter_bit_in_64 = '{:064b}'.format(
                    int('1' * lenth, 2) << (start % 8))

                for i, j in enumerate(range(start, end, 8)):
                    byte_index, _ = divmod(j, 8)
                    filter_bit_in_bin = filter_bit_in_64[::- \
                        1][i * 8:(i + 1) * 8][::-1]
                    filter_bit_in_hex = '0x%02X' % int(
                        filter_bit_in_64[::-1][i * 8:(i + 1) * 8][::-1], 2)
                    shift_dir = '<<' if i == 0 else '>>'
                    shift_num = len(filter_bit_in_bin) - \
                                len(filter_bit_in_bin.rstrip('0'))
                    shift_num = shift_num if i == 0 else 8

                    hacg_can_c_file.write(
                        f'    {msg.name}_buf[{byte_index}] |= ((uint8_t)(pfc_{sig.name.split("_")[-1]} {shift_dir} {shift_num}u) & {filter_bit_in_hex}u);\n')
            ...
            # pfc_PM_Motor_Torque |= ((uint16_t)(PMtoVMS_STATUS1_buf[1] &
            # 0xFFu) << 8u);
            hacg_can_c_file.write(
                f'void Can0Msg_Unpack_{hex(msg.frame_id)}(void)\n'
                f'{{\n')
            for sig in msg.signals:
                ...
                for i, j in enumerate(range(start, end, 8)):
                    ...
                    # print(shift_num)
                    hacg_can_c_file.write(
                        f'    pfc_{sig.name.split("_")[-1]} {assignment_symbol} (({sig_type})({msg.name}_buf[{byte_index}] & {filter_bit_in_hex}u) {shift_dir} {shift_num}u);\n')
